  ...
  if (shouldUseNew && serviceRoot) {
    ...
    if (rootFiles.length) {
      ...
      const items = rootFiles.map((file) => {
        const publicUrl = "/api/new/" + encodeURI(serviceToNewFolder[service]) + "/" + encodeURIComponent(file)
    ...
    for (const section of sectionFolders) {
      ...
      const items = files.map((file) => {
        const publicUrl = "/api/new/" + encodeURI(serviceToNewFolder[service]) + "/" + encodeURI(section) + "/" + encodeURIComponent(file)
        return { src: publicUrl, filename: file }
      })
